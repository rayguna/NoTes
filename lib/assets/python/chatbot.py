try:
    ...
    print(f"Error: {e}. This feature is temporarily disabled due to missing dependencies.")
    sys.exit(1)

# Check if the OpenAI API key is available
api_key = os.getenv('MY_GPT2_KEY')
if not api_key:
    print("OpenAI API key is not set. This feature is temporarily disabled.")
    sys.exit(1)
...
def fetch_notes_by_topic_name(topic_name, input_text):
    ...
    filtered_notes = [
        f"Title: {title}\nContent: {content}"
        for title, content, topic in notes if topic.lower() == topic_name.lower()
    ]
...
if __name__ == "__main__":
    try:
        ...
        print("This feature is temporarily disabled due to low credits. Response: ", str(e))
